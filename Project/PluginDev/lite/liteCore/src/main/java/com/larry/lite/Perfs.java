...
public class Perfs {
        ...
        if (!file.exists()) {
            ...
            if (TextUtils.isEmpty(content)) {
                ...
                if (TextUtils.isEmpty(content)) {
                    ...
                    LiteLog.i("tcp_snd: %s", new Object[] {content});
        ...
        if (!file.exists()) {
            ...
            if (TextUtils.isEmpty(content)) {
                ...
                if (TextUtils.isEmpty(content)) {
                    ...
                    LiteLog.i("tcp_rcv: %s", new Object[] {content});
                    return Long.parseLong(content);
                }
            }
        }
    ...
    private static Perfs.ProcessStats getProcessNetworkStats(Perfs.ProcessStats ps) {
        ...
        LiteLog.i("SUPPORT_FILE_STAT_NETWORK = %d", new Object[] {Integer.valueOf(SUPPORT_FILE_STAT_NETWORK)});
        if (SUPPORT_FILE_STAT_NETWORK == 0) {
            try {
                ps.txBytes = getTxBytes(uid);
                ps.rxBytes = getRxBytes(uid);
                SUPPORT_FILE_STAT_NETWORK = 1;
                LiteLog.i("uid =%d, getTxBytes =%d, getRxBytes =%d",
                ...
                LiteLog.i("uid =%d, TrafficStats getUidTxBytes =%d, getUidRxBytes =%d",
                        new Object[] {Integer.valueOf(uid), Long.valueOf(ps.txBytes), Long.valueOf(ps.rxBytes)});
            }
    ...
    public static Perfs.MemoryInfo getProcessMemoryInfo(Context context) {
        ...
        mi.totalMem = mi.PSS;
        LiteLog.i("mem: uss=%dKB, pss=%dKB, rss=%dKB, free=%dKB", new Object[] {Long.valueOf(mi.USS),
                Long.valueOf(mi.PSS), Long.valueOf(mi.RSS), Long.valueOf(mi.availMem)});
        return mi;
    }
