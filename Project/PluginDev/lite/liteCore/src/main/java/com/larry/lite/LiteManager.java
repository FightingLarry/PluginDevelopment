...
public class LiteManager implements ILiteLifecycleCallback, Callback {
    ...
    public LiteManager(LiteContext context) {
        this.mContext = context;
        this.mIoHandler = new LiteManager.EngineHandler();
    ...
    public void setRunLoop(Looper looper) {
        this.mIoHandler = new LiteManager.EngineHandler(looper);
    }
    ...
    public void pumpEvent(LiteEvent event, Object extra) {
        ...
        if (configuration == null) {
            ...
            LiteManager.EventAction action = new LiteManager.EventAction(this, event);
            if (extra != null) {
    ...
    public void onPluginCreate(LiteStub plugin) {
        LiteLog.v("%s onPluginCreate", new Object[] {"LiteManager"});
    }
    ...
    int updateConfiguration(boolean ignoreExpired) {
        if (this.isUpdatingConfiguration) {
            ...
            LiteLog.w("config not expired! last update time: %s",
                    this.mContext.getConfiguration().getLastUpdateTimestamp());
            return 2;
        } else {
    ...
    private static class NetworkChangedDuctor implements NetworkHelper.NetworkInductor {
        WeakReference<LiteManager> mEngine;

        public NetworkChangedDuctor(LiteManager engine) {
        ...
        public void onNetworkChanged(NetworkStatus networkStatus) {
            if (!networkStatus.equals(NetworkStatus.NetworkNotReachable)) {
                LiteManager engine = (LiteManager) this.mEngine.get();
                if (engine != null) {
    ...
    private static class EventAction implements Runnable {
        ...
        EventAction(LiteManager engine, LiteEvent event) {
            this.mEngine = new WeakReference(engine);
            this.mEvent = event;
        }
        ...
        public boolean equals(Object o) {
            if (this == o) {
                ...
                LiteManager.EventAction that = (LiteManager.EventAction) o;
    ...
    private class EngineHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == MSG_UPDATE_CONFIGURATION) {
                LiteManager.this.mPendingAction = (Runnable) msg.obj;
                boolean ignore = msg.arg1 > 0;
                int ret = LiteManager.this.updateConfiguration(ignore);
                if (ret <= -1 && ret > -10) {
                    LiteManager.this.mRetryTimesForFailed++;
                    if (LiteManager.this.mRetryTimesForFailed >= 3) {
                        LiteLog.w("retry too many times: %d",
                                new Object[] {Integer.valueOf(LiteManager.this.mRetryTimesForFailed)});
                        return;
                    }
                ...
                if (msg.arg1 == 1) {
                    ...
                    LiteManager.this.mContinueStating = false;
                    LiteManager.this.mStats.end(msg.arg2);
                } else if (msg.arg1 == 3) {
                    LiteManager.this.mContinueStating = false;
                    LiteManager.this.mStats.beginRelease();
                } else {
                    LiteManager.this.mStats.updatePerfs();
                }
                ...
                LiteManager.this.reportStats(stub);
            }
